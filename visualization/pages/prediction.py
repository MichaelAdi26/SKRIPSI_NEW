...
def update_prediction(n_clicks,selected_province, selected_freq, selected_model, year_range):
    ...
    if year_range[0] <= 2023 and year_range[1] > 2023:
        ...
        # --- Data historis (2021–2023) --- untuk All Parameter / Features:
        historical_df = train_df[(train_df["Date"].dt.year >= year_range[0]) & (train_df["Date"].dt.year <= 2023)].copy()
        # --- Data historis (2021–2023) --- untuk Top 5 Parameter / Features:
        ...
        if actual_model_name in ["Linear Regression", "Random Forest", "XGBoost", "KNN","LightGBM","CatBoost","Extra Trees","SVR"]:
            ...
            X_hist_reshaped = X_future.reshape((X_hist_scaled.shape[0], X_hist_scaled.shape[1], 1))
        ...
        # Historical (rata-rata per tanggal)
        historical_mean = historical_df.groupby("Date")["Demand"].mean().reset_index()
        historical_mean.sort_values("Date", inplace=True)

        # Predicted future (rata-rata per tanggal)
        future_mean = future_df.groupby("Date")["Prediction"].mean().reset_index()
        future_mean.sort_values("Date", inplace=True)

        # Combine: actual + predicted untuk visualisasi untuk ALL Parameter
        fig_line = go.Figure()
        fig_line.add_trace(go.Scatter(x=historical_mean["Date"], y=historical_mean["Demand"], mode='lines', name="Actual"))
        fig_line.add_trace(go.Scatter(x=future_mean["Date"], y=future_mean["Prediction"], mode='lines', name="Predicted"))
        fig_line.update_layout(
            title='Predicted Electricity Demand with All Importance Features/Parameter (in KWh)', title_x=0.5, xaxis_title='Date',
            yaxis_title='Demand (KWh)', height=500,
        ...
        # Untuk mengukur nilai RMSE, MAE, dan R² saja:
        if actual_model_name in ["Linear Regression", "Random Forest", "XGBoost", "KNN","LightGBM","CatBoost","Extra Trees","SVR"]:
            y_pred_eval2 = model_top5.predict(X_hist2)
        elif actual_model_name in ["LSTM", "CNN-1D", "GRU", "MLP"]:
            X_future_reshaped2 = X_future2.reshape((X_hist2.shape[0], X_hist2.shape[1], 1))
            y_pred_eval2 = model_top5.predict(X_future_reshaped2)
        else:
            y_pred_eval2 = model_top5.predict(X_hist2).flatten() # NO expand_dims
        ...
        # Combine: actual + predicted untuk visualisasi untuk ALL Parameter
        fig_line2 = go.Figure()
        fig_line2.add_trace(go.Scatter(x=historical_mean["Date"], y=historical_mean["Demand"], mode='lines', name="Actual"))
        fig_line2.add_trace(go.Scatter(x=future_mean["Date"], y=future_mean["Prediction"], mode='lines', name="Predicted"))
        fig_line2.update_layout(
            title='Predicted Electricity Demand with Top 5 Highest Importance Features/Parameter (in KWh)', title_x=0.5, xaxis_title='Date',
            yaxis_title='Demand (KWh)', height=500,
            legend=dict(x=1, y=1, xanchor='right', yanchor='top'),
            margin=dict(l=40, r=40, t=80, b=40)
        )
